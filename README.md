...
- 出題設定の「効果音」がONの場合、選択肢クリック後にWeb Audio APIで正解時は短い上昇音、不正解時は短い低音を控えめに鳴らします（初期値ON）。自動読み上げとは別設定です。
- 効果音は音声ファイルを使わず、Web Audio API非対応環境では何もせず学習機能を継続します。
- 次の問題や読み込み状態へ移る前に `speechSynthesis.cancel()` で前の音声を停止します。効果音再生のためには `speechSynthesis.cancel()` を呼びません。
- iPhone Safariなどで自動再生が制限されても、手動ボタンや選択肢タップ直後の効果音が動きやすいように外部音声APIやAPIキーは使いません。
