...
`study-app` では Web Speech API の `speechSynthesis` を使い、外部音声ファイルなしで問題文の英語を読み上げます。

- 英単語モードでは `question` の英単語、チャンクモードでは `question` の英語チャンク、英文和訳モードでは `question` の英文を読み上げます。
- 問題表示時に1回だけ自動読み上げします。ホーム画面の「問題を自動で読み上げる」でON/OFFできます。
- 設定は `localStorage` に保存され、初期値はONです。
- 問題画面の「🔊 もう一度聞く」ボタンは、自動読み上げOFF時でも現在の問題文を再生できます。
- 読み上げは `en-US`、速度は少しゆっくりめの `0.85` です。
- iPhone Safariなどで自動再生が制限される場合は、ボタン操作で再生してください。
