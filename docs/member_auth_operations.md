...
第1段階として、グループ管理と会員所属を追加しました。`002_groups.sql`を適用すると、`groups`と`group_members`が作成されます。代表管理者／一般管理者は`/admin/groups/`からグループを作成し、既存会員を複数のグループへ所属させられます。

CSV／Excel会員一括登録・出力、テスト作成・配信・受験・採点・履歴・成績出力は、今後の第2弾の段階で追加します。
