- 2026-07-19: 第2弾の第1段階として、グループ管理と会員所属を追加。代表／一般管理者がグループ名・説明を作成／編集し、既存会員を検索して複数選択できる。削除は履歴を残すアーカイブ方式とし、作成・編集・所属差分・アーカイブを監査ログへ記録する。`002_groups.sql`で`groups`と`group_members`を追加。
- 2026-07-17: PostgreSQL、前方適用型マイグレーション、代表／一般／閲覧管理者、会員ID自動発行、bcryptパスワード、AES-256-GCMによる初期・仮パスワード一時表示、DB保存型30日セッション、10回失敗時15分ロック、権限別API、監査ログ、最小管理者・会員画面を追加。DB未設定時は認証機能だけ503となり、既存RPG・study-app・問題／音声APIは継続起動する。第2弾のグループ・テスト配信・受験履歴は未実装。
