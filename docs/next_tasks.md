...
## study-app 第2段階・第3段階の残作業

- 第2段階は実装済み: `.xlsx` 一括アップロードAPI `POST /api/questions/upload-workbook`、旧 `POST /api/questions/upload` 無効化、4シート完全一致チェック、Render保存データ `schema_version: 2` 対応まで完了。
- 第3段階: `question_key` の接頭辞/桁数などの形式チェック、重複チェック、D〜G列重複チェックを追加する。
- 第3段階: 専用エラーボックスと最大20件のエラー一覧表示を追加する。
- 実ブラウザで `phrase` モード、音声読み上げ、効果音ON/OFF、復習、レベル範囲、ランダム出題を確認する。


## 第3段階候補
- `question_key` の `w000001` / `c000001` / `p000001` / `s000001` 形式チェック。
- シート内 `question_key` 重複チェック。
- B列 `level` の厳格チェック。
- D〜G列の重複チェックと選択肢正規化。
- 最大20件の行番号付き検証エラー表示。
- study-app専用エラーボックス追加。
- 実ブラウザで4シートExcelアップロードの手動確認とスクリーンショット取得。
