...
### study-appの3シートExcelブック読み込み

`study-app/script.js` は、アップロードされたExcelブックに `★英単語テスト_001_生成`、`★チャンク_001_生成`、`★英文和訳_001_生成` の3シートがすべて存在する場合、先頭シート単一読み込みではなく複数モードブックとして扱います。各シートは既存のA〜L標準列へ正規化し、`word` / `chunk` / `definition` のモード別行データとして保持します。

ブラウザ上では `state.localModeRows` に3モード分を保持し、モード切替時は該当モードのシート由来データだけを `normalizeQuestions` に渡します。Render APIが利用できる場合は、既存互換を保つため `/api/questions/upload` へモード別CSVとして3回送信し、サーバー側の保存形式は従来の `modes.{mode}` を維持します。指定3シートがそろわないExcelは、既存互換のため先頭シート単一Excelとして読み込みます。
