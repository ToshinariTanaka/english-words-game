...
(async () => {
  ...
  try {
    ...
    const chunk = await request('GET', '/api/questions/current?mode=chunk');
    assert.strictEqual(chunk.status, 200, chunk.text);
    assert.strictEqual(chunk.json.mode, 'chunk');
    assert.strictEqual(chunk.json.rows[0].question_key, 'c000001');
    ...
    assert.notStrictEqual(phrase.json.rows[0].question_key, chunk.json.rows[0].question_key);
