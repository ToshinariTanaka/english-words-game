...
for (const name of ['★英単語', '★チャンク', '★英文和訳']) {
...
assert.ok(source.includes('parseWorkbookModeRows(await file.arrayBuffer(), uploadMode, { officialOnly: true })'), 'study-appの正式アップロードは公式3シートExcelとして解析する');
assert.ok(source.includes('正式アップロードは .xlsx の3シートExcelのみ対応です'), '公式3シート以外のエラーを表示する');
assert.ok(source.includes('fetch(`${API_BASE}/api/questions/upload-workbook`'), '3シートExcelは一括アップロードAPIへ送信する');
